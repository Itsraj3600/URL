...
class IndexHistoryDB:
    ...
    async def initialize(cls):
        ...
        try:
            ...
            # TTL index to auto-delete old records (2 years)
            await collection.create_index([("finished", 1)], expireAfterSeconds=63072000)

            logger.info(f"[IndexHistory] Initialized {cls.COLLECTION_NAME} collection")
        except Exception as e:
            logger.error(f"[IndexHistory] Failed to initialize collection: {e}")
    ...
    async def save_job(
        ...
        try:
            ...
            record = {
                ...
                "type": job_type,  # "manual", "auto", "scheduled"
                "indexed": indexed,
            ...
            logger.info(f"[IndexHistory] Saved job {job_id} - {indexed:,} indexed")
            return bool(result.inserted_id)
        except Exception as e:
            logger.error(f"[IndexHistory] Failed to save job: {e}")
            return False
    ...
    async def get_job_history(
        ...
        try:
            ...
            logger.error(f"[IndexHistory] Failed to get job history: {e}")
            return []
    ...
    async def get_job(cls, job_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(f"[IndexHistory] Failed to get job {job_id}: {e}")
            return None
    ...
    async def get_stats_by_channel(cls, channel_id: int, days: int = 7) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"[IndexHistory] Failed to get channel stats: {e}")
            return {}
        ...
        try:
            ...
            if result:
                ...
                return {
                    ...
                    "total_processed": total_processed,
                    "success_rate": (stats.get("total_indexed", 0) / total_processed * 100) if total_processed > 0 else 0,
                    "avg_speed": stats.get("avg_speed", 0),
            ...
            logger.error(f"[IndexHistory] Failed to get global stats: {e}")
            return {}
    ...
    async def count_jobs(cls, days: int = 7) -> int:
        ...
        try:
            ...
            logger.error(f"[IndexHistory] Failed to count jobs: {e}")
            return 0
    ...
    async def delete_old_records(cls, days: int = 730) -> int:
        ...
        try:
            ...
            result = await collection.delete_many({
                "finished": {"$lt": cutoff_date}
            })

            logger.info(f"[IndexHistory] Deleted {result.deleted_count} old records")
            return result.deleted_count
        except Exception as e:
            logger.error(f"[IndexHistory] Failed to delete old records: {e}")
            return 0
